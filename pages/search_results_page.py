...
class SearchResultsPage(BasePage):
    ...
    def getProductCards(self) -> list[dict]:
        ...
        for i in range(product_cards.count()):
            ...
            title = title_locator.text_content().strip() if title_locator.count() > 0 else None
            ...
            price = price_locator.text_content().strip() if price_locator.count() > 0 else "Not available"
            
            # Rating
            rating_locator = product_card.locator('.a-icon-alt')
            rating = rating_locator.text_content().strip() if rating_locator.count() > 0 else None
            
            # Review count
            review_locator = product_card.locator('[aria-label*="ratings"]')
            review_count = review_locator.get_attribute('aria-label') if review_locator.count() > 0 else None
